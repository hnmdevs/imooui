...
async function convertMdxToMarkdown(
  ...
  try {
    ...
    convertedContent = convertedContent.replace(
      ...
      (match: string, name: string) => {
        try {
          ...
          return `See the interactive demo at: [${name}](https://imooui.imoogleai.xyz/docs/components/${slug.join("/")})\n\n`
        } catch (error) {
    ...
    // Add footer
    markdown += `\n\n---\n\n*This documentation is also available in [interactive format](https://imooui.imoogleai.xyz/docs/components/${slug.join("/")}).*\n`
...
function convertLinksToMarkdown(content: string): string {
  ...
  return content.replace(
    ...
    (match, href, linkText) => {
      ...
      let fullUrl = href
      if (href.startsWith('/')) {
        fullUrl = `https://imooui.imoogleai.xyz${href}.md`
      }
